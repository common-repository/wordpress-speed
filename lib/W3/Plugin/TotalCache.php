        ...
        if ($buffer != '' && w3_is_xml($buffer)) {
            if (w3_is_database_error($buffer)) {
                ...
                if ($this->_config->get_string('common.support') != '' || $this->_config->get_boolean('common.tweeted')) {
                    $buffer .= sprintf("\r\n<!-- Served from: %s @ %s by WordPress Speed -->", w3_escape_comment($host), $date);
                } else {
                    $strings = array();
                    ...
                    $buffer .= "\r\n<!-- Performance optimized by WordPress Speed. Learn more: http://www.w3-edge.com/wordpress-plugins/\r\n";
