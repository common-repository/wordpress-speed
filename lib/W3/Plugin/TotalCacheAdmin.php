...
class W3_Plugin_TotalCacheAdmin extends W3_Plugin {
    ...
    function admin_menu() {
        ...
        add_menu_page('Performance', 'Performance', 'manage_options', 'w3tc_general', '', plugins_url('wordpress-speed/pub/img/logo_small.png'));

        $submenu_pages = array();

        foreach ($pages as $slug => $titles) {
            $submenu_pages[] = add_submenu_page('w3tc_general', $titles[0] . ' | WordPress Speed', $titles[1], 'manage_options', $slug, array(
                &$this,
                'options'
            ));
        }
    ...
    function wp_dashboard_setup() {
        ...
        if ($this->_config->get_boolean('widget.pagespeed.enabled')) {
            wp_add_dashboard_widget('w3tc_pagespeed', 'WordPress Speed: Google Page Speed Report', array(
                &$this,
                'widget_pagespeed'
            ), array(
                &$this,
    ...
    function admin_notices() {
        ...
        if (!w3_is_preview_config() && !file_exists(W3TC_CONFIG_PATH)) {
            $errors[] = sprintf('<strong>WordPress Speed Error:</strong> Default settings are in use. The configuration file could not be read or doesn\'t exist. Please %s to create the file.', $this->button_link('save the settings', wp_nonce_url(sprintf('admin.php?page=%s&w3tc_save_config', $this->_page), 'w3tc')));
        }
    ...
    function enable_cookie_domain() {
        ...
        if ($this->is_cookie_domain_define($config_data)) {
            ...
            $new_config_data = preg_replace('~<\?(php)?~', "\\0\r\ndefine('COOKIE_DOMAIN', '" . addslashes($cookie_domain) . "'); // Added by WordPress Speed\r\n", $config_data, 1);
        }
